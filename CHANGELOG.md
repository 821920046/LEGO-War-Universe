...
## V4.3 — AI 编排稳定性修复（修 HTTP 502）

**问题**：前端报「AI 编排未成功（HTTP 502）」。502 来自 Cloudflare 网关而非本项目代码——AI 函数把 11 个模型按 45 秒超时串行重试，最长可达 8 分钟，远超 Pages Function 的执行上限，函数被平台强杀，响应体为空，所以前端只能显示裸的 HTTP 502。

**修复**

1. **硬性时间预算**：整条故障转移链共享 24 秒总预算（`DEADLINE_MS`），单个模型 14 秒（`TIMEOUT_MS`），最多尝试 4 个模型（`MAX_ATTEMPTS`）。剩余时间不足 4 秒即停止，绝不再被平台中断。
2. **提示词瘦身**：资产表在服务端压缩（字段改为 `id/k/name/zh/s/f`，长文本截断），载荷体积下降约 60%，免费模型不再因上下文过长而 400/429/超时。动作短语上限 200 → 120。
3. **节拍计划 + 服务端展开**：长片不再要求模型一次输出 30+ 个镜头的 JSON（必然被 token 上限截断）。改为让 AI 输出最多 12 个关键节拍，服务端按比例展开为准确的 N × 8 秒镜头。
4. **JSON 模式兼容**：部分免费模型不支持 `response_format: json_object` / `responseMimeType`，遇到 4xx 且报错含相关关键字时，自动以纯文本模式重试同一模型。
5. **可读的错误**：AI 失败改为返回 HTTP 200 + `{ok:false, error, attempts[]}`，前端逐条显示每个模型的失败原因。今后若再见到真正的 502，即可断定是平台层问题。
6. **体检接口**：`GET /api/compose?probe=1` 逐个真实调用链上模型，返回每个模型的 `ok`、耗时与失败原因，用于一眼定位是哪家密钥/模型出问题。
7. 版本号与构建时间戳同步更新为 V4.3。

## V4.4 — 修复免费档 413 / 动态提示词压缩

**真实报错（V4.3 暴露出来的）**

- `groq/llama-3.1-8b-instant` → **HTTP 413**：`Request too large … tokens per minute (TPM): Limit 6000`
- `gemini/gemini-2.0-flash-lite` → **HTTP 429**：`You exceeded your current quota`

即：不是代码错，而是 **提示词体积超出了免费档的每分钟 token 预算**（Groq 仅 6000 TPM），以及 Gemini 免费额度用尽。

**修复**

1. **三档提示词体积（full / lite / min）**：按供应商选档。Groq 直接使用 `lite`；其他供应商从 `full` 开始，一旦报 413 / context length / TPM 类错误，自动逐级缩小到 `lite` → `min` 重试同一模型。
2. **主题相关度筛选**：不再把 392 个资产全部上传。服务端用中文 2-gram + 英文词对资产评分，优先保留与主题相关的，剩下按分类配额补齐（角色/载具/武器/道具/环境/运镜/光光/色彩/特效/声音各自有上限）。
3. **行格式代替 JSON**：资产表改为 `id|name|nameZh|series|faction` 每行一条，相同信息量下 token 再降约 40%。动作短语按档限量（full 60 / lite 24 / min 0）并改用 ` / ` 拼接。
4. **失败快、重试多**：单模型超时 12 秒，最大尝试模型数 4 → **8**（413/429 类错误是瞬时返回的，不占预算），仍受 24 秒总预算保护。
5. 响应新增 `level` 字段，方便看出最终是哪个体积档跑成功的。
6. 运维面板新增 413 / 429 的中文排错说明。

**体积对比（估算）**：V4.2 约 100 KB → V4.3 约 40 KB → V4.4 `full` 约 12 KB（≈3.5k tokens）、`lite` 约 5 KB（≈1.5k tokens）、`min` 约 2.5 KB。Groq 的 6000 TPM 至此完全装得下。

## V4.5 — 免费账号池（多个 OpenRouter 账号轮询）

**目标**：同一供应商可配多个免费账号密钥，额度叠加，轮询使用，遇限流自动换号。

1. **密钥池读取**（三种写法可混用，自动去重）：
   - `OPENROUTER_API_KEYS` = `sk-or-a, sk-or-b, sk-or-c`（逗号 / 分号 / 空格 / 换行分隔）
   - `OPENROUTER_API_KEY_1` … `OPENROUTER_API_KEY_12`（一个变量一个账号）
   - `OPENROUTER_API_KEY`（旧写法，仍生效）
   对所有供应商通用：`GEMINI_API_KEYS`、`GROQ_API_KEY_1` 等。
2. **轮询起点**：每个供应商维护独立的密钥游标，每次请求从不同账号起步，不会总是只磨第一个账号。
3. **遇限流即换号**：某账号返回 429 / 401 / 402 / 403（或报错含 quota / rate limit / insufficient credit / invalid key）时，**用同一个模型换下一个账号重试**，而不是直接跳到下一个模型。可选 `KEY_TRIES` 限制单模型最多消耗几个账号。
4. **413 缩小与换号互不干扰**：体积缩小（full→lite→min）与账号轮换共同工作，均受 24 秒总预算保护。
5. **响应可读**：失败明细里的供应商标记为 `openrouter#2` 形式，直接看出是哪个账号失败；成功时返回 `account` 与 `accountPool`。
6. **两个体检接口**：
   - `GET /api/compose?probe=1` — 逐个模型体检
   - `GET /api/compose?probe=keys` — **逐个密钥体检**，返回密钥后四位、是否可用、耗时与失败原因
7. **前端运维面板**：新增「账号池」行，显示共多少个密钥及各供应商分布；说明文档新增第 11、12 条。
